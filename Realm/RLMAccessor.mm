...
static id RLMAccessorStandaloneGetter(RLMProperty *prop, RLMAccessorCode accessorCode) {
...
static id RLMAccessorStandaloneSetter(RLMProperty *prop, RLMAccessorCode accessorCode) {
    // only override getters for RLMArray and linking objects properties
    if (accessorCode == RLMAccessorCodeArray) {
        NSString *propName = prop.name;
        NSString *objectClassName = prop.objectClassName;
...
Class RLMAccessorClassForObjectClass(Class objectClass, RLMObjectSchema *schema, const char *name) {
